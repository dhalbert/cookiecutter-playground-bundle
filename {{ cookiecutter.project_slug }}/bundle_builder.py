...
dirs = {
    ...
    '8.x':    os.path.join('build', repo_name, 'CircuitPython 8.x'),
    '9.x':    os.path.join('build', repo_name, 'CircuitPython 9.x'),
    '8_lib':  os.path.join('build', repo_name, 'CircuitPython 8.x', 'lib'),
...
url9 = cfg['9.x']
cache = os.path.join('build', 'cache')
zip8path = os.path.join(cache, basename(url8))
zip9path = os.path.join(cache, basename(url9))
if not isfile(zip8path):
    print("downloading", url8)
    run(f"cd {cache} && curl -L -O {url8}")
...
def extract_libs(zip_path, dst_dir, lib_names):
    ...
    for i in src_items:
        ...
        # "adafruit-circuitpython-bundle-8.x-mpy-20240625/lib/adafruit_midi/note_on.mpy",
...
extract_libs(zip8path, dirs['8_lib'], cfg['lib'])
